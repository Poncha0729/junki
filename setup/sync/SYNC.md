...
現PC（AIメイン）と新PC（Windows）を、**壊さずに**同じ状態で使うための取り決めです。
...
| | 現PC（母艦） | 新PC |
|---|---|---|
| 主な用途 | **AI作業メイン**（Claude Code、生成処理、調査） | 開発・事務・外出前の確認 |
| 常駐 | Claude デスクトップ / Claude Code | VS Code |
| 重い処理 | こちらに寄せる | 避ける |

AI にコードを書かせるのは現PCに寄せ、**成果物は必ず GitHub 経由**で新PCに渡します。
「新PCの画面で直接コピペ」は履歴が残らないので避けてください。
